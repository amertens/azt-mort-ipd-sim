...
run_sim_par <- function(full_res = NULL,
  ...
  # Run analyses
  results <- run_analysis_one_step(studies_data=sim_study_data, adjusted=adjusted, run_tmle=run_tmle)
...
clusterExport(cl, c("run_sim_par","study_params","simulate_trial_data","generate_covariates",
                    "run_analysis_one_step","iter_range"), envir=environment())
...
full_res=NULL
 full_res=readRDS(here("results/sim_results_interim_par_1step_RE.rds"))
...
for(i in 1:20){
  ...
  saveRDS(full_res, file=here("results/sim_results_interim_par_1step_RE.rds"))
}

length(unique(full_res$iteration))

saveRDS(full_res, file=here("results/sim_results_par_1step_RE.rds"))
stopCluster(cl)
