...
results_RE_1step <- readRDS(file=here("results/sim_results_interim_par_1step_RE.rds")) %>% 
  mutate(study="pooled", group="main", level="main", adjusted=TRUE, hr=NA, tmle_log_rr=NA, tmle_ate=NA, hr_se=NA, tmle_rr_log_se=NA, tmle_ate_se=NA,hr_pval=NA, tmle_rr_pval=NA, tmle_ate_pval=NA)
results_FE_1step <- readRDS(file=here("results/sim_results_interim_par_1step_FE.rds")) %>%  filter(level=="main") %>%
  mutate(study="pooled", group="main", level="main", adjusted=TRUE, crude_rr=NA, hr=NA, tmle_log_rr=NA, tmle_ate=NA, hr_se=NA, tmle_rr_log_se=NA, tmle_ate_se=NA,hr_pval=NA, tmle_rr_pval=NA, tmle_ate_pval=NA)
...
res_FE_2step <- d_FE_2step %>% group_by(level, metric, adjusted) %>% 
  filter(level=="main") %>%
  summarise(n=n(),
  ...
  filter(!is.na(abs_bias), metric %in% c("cid", "cir")) %>%
...
res_RE_2step <- d_RE_2step %>% group_by(level, metric, adjusted) %>%
  ...
  summarise(n=n(),
            ...
            coverage=mean(lower<=true_value & true_value<=upper)*100,
            O_coverage=mean(effect -1.96*sd(effect )< true_value & true_value < effect +1.96*sd(effect ))*100) %>%
  filter(!is.na(abs_bias), metric %in% c("cid", "cir","tmle_ate")) %>%
...
res %>% filter(metric=="cir") %>% select(pooling, everything())

res %>% filter(metric=="cid"|metric=="tmle_ate", adjusted, grepl("RE",pooling)) %>% select(pooling, everything())
res %>% filter(metric=="cir", adjusted, grepl("RE",pooling)) %>% select(pooling, everything())
